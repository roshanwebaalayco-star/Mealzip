...
export const MEDICATION_RULES: Record<string, MedicationRule & { cross_slot_conflict?: any; day_wide_restrictions?: any; regional_flag?: string }> = {
  metformin: {
    ...
    match_keywords: [
      "metformin", "glucophage", "glycomet", "obimet", "glyciphage",
      "gluformin", "carbophage", "riomet",
    ],
    match_regex: /metformin|glucophage|glycomet|obimet|glyciphage/i,
    slot_constraints: [
      {
        slot: "breakfast",
        must_have_food: true,
        ...
        positive_requirements: [
          "Substantial solid food — roti/paratha/poha/idli — NOT just tea or juice",
        ],
        timing_instruction:
          "Metformin MUST be taken WITH a solid meal to reduce GI side effects. " +
          "Breakfast must be substantial (not just tea). If taken at dinner, dinner must also be solid.",
      },
    ],
    dynamic_slot_rule: {
      forbidden_ingredients: [],
      must_have_solid_food: true,
      timing_instruction:
        "Member takes Metformin at THIS slot. Ensure a solid meal (roti/rice/paratha) is served — " +
        "not a light snack or liquid-only meal.",
    },
    weekly_monitor: null,
    scheduling_note:
      "If Metformin is taken twice daily (common), ensure both breakfast AND dinner are substantial solid meals. " +
      "Light meals or tea-only breakfasts will cause GI distress (nausea, cramps).",
    clinical_reason:
      "Metformin causes gastrointestinal distress (nausea, diarrhoea, cramping) when taken on an empty stomach or with inadequate food. " +
      "The extended-release formulation specifically requires food to slow gastric emptying and modulate absorption.",
    harmony_score_addition: 3,
  },

  iron_supplement: {
    drug_id: "iron_supplement",
    display_name: "Iron Supplement (Anaemia)",
    match_keywords: [
...
const TIMING_PATTERNS: Array<{
  ...
  { pattern: /with[\s_]?food|after[\s_]?food|after[\s_]?eating/i,             slot: "breakfast", relation: "with" },
];
...
export function buildGuardrailStrings(
  memberName: string,
  rule: MedicationRule & { cross_slot_conflict?: any; day_wide_restrictions?: any },
  ...
  for (const slotConstraint of rule.slot_constraints) {
    ...
    if (slotConstraint.must_have_food || slotConstraint.must_have_solid_food) {
      parts.push(
  ...
  if (rule.day_wide_restrictions) {
    const dwd = rule.day_wide_restrictions;
    directives.push(
  ...
  if (rule.cross_slot_conflict) {
    const csc = rule.cross_slot_conflict;
