...
import {
  ...
  Sparkles, Minus, Plus, Camera, CheckCircle2, Scale, Leaf, X,
} from "lucide-react";
...
export default function MealGenPopup({ open, onClose, onGenerated }: MealGenPopupProps) {
  ...
  }, [open, familyId, members.length, defaultBudget, initializedForOpen]);
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
              ...
              <div className="rounded-xl border border-border bg-muted/30 p-3">
                ...
                <textarea
                  ...
                  className="w-full resize-none rounded-lg border border-border bg-white dark:bg-zinc-800 text-xs px-3 py-2 min-h-[48px] focus:outline-none focus:ring-1 focus:ring-primary/50"
                />
              </div>

              {members.length > 0 && (
                <div>
                  <p className="text-xs font-semibold text-foreground mb-2">{t("Member Check-In", "सदस्य स्थिति")}</p>
                        ...
                        <div key={member.id} className="border border-border rounded-xl overflow-hidden">
                            ...
                            <div className="flex items-center gap-1.5">
                              {memberOv.feeling_this_week && (
                                <span className="text-[10px] bg-primary/10 text-primary px-1.5 py-0.5 rounded-full truncate max-w-[60px]">
                                  {memberOv.feeling_this_week.slice(0, 10)}
                                </span>
                              )}
                          ...
                          <AnimatePresence>
                            {isExpanded && (
                              ...
                              >
                                <div className="px-3 pb-3 pt-0 space-y-2.5 border-t border-border bg-muted/20">
                                  {showWeight && (
                                    <div className="pt-2.5">
                                      ...
                                      <Input
                                        ...
                                        placeholder="e.g. 68"
                                        className="mt-1 h-8 rounded-lg text-xs w-24"
                                      />
                                    </div>
                                  )}

                                  <div className={showWeight ? "" : "pt-2.5"}>
                                    <Label className="text-[10px] font-semibold text-muted-foreground">{t("Feeling this week?", "इस हफ्ते कैसा महसूस?")}</Label>
                                    <Input
                                  ...
                                  <div>
                                    <Label className="text-[10px] font-semibold text-muted-foreground">{t("Fasting days", "उपवास")}</Label>
                                  ...
                                  {showNonVeg && (
                                    <div>
                                      <Label className="text-[10px] font-semibold text-muted-foreground flex items-center gap-1">
                                        <Leaf className="w-2.5 h-2.5 text-orange-500" />
                                      ...
                                      {(memberOv.nonveg_days_override ?? []).length > 0 && (
                                        ...
                                        >
                                          <SelectTrigger className="mt-1.5 h-7 rounded-lg text-[10px] border-border"><SelectValue /></SelectTrigger>
                                          <SelectContent>
                                  ...
                                  <div className="grid grid-cols-2 gap-2">
                                    <label className="flex items-center gap-1.5 cursor-pointer">
                                      ...
                                      />
                                      <span className="text-[10px]">{t("Needs tiffin", "टिफिन")}</span>
                                    </label>
                                    <Select
                                    ...
                                    >
                                      <SelectTrigger className="h-7 rounded-lg text-[10px] border-border"><SelectValue /></SelectTrigger>
                                      <SelectContent>
              ...
              <div className="flex items-center gap-3 p-2.5 rounded-xl bg-amber-50 border border-amber-200">
                <label className="flex items-center gap-2 cursor-pointer flex-1">
                  ...
                  />
                  <div>
                    <p className="text-xs font-semibold text-amber-900">{t("Fasting Week", "व्रत सप्ताह")}</p>
                    <p className="text-[10px] text-amber-700">{t("Sabudana, kuttu, fruits etc.", "साबूदाना, कुट्टू, फल आदि")}</p>
                  </div>
                </label>
              </div>
            </div>
