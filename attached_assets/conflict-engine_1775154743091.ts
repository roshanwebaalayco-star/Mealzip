// =============================================================================
// NutriNext ParivarSehat — Conflict Resolution & Constraint Satisfaction Engine
// The deterministic core. Runs in < 1 second before any AI call.
// Answers: "Given this family this week, what are the exact constraints
// and how do we resolve every conflict?"
//
// Priority hierarchy (hardcoded, never overridden):
//   Level 1 — Allergy (absolute)
//   Level 2 — Religious / cultural rule (absolute)
//   Level 3 — Medication interaction window (absolute for that meal slot)
//   Level 4 — Clinical condition management
//   Level 5 — Personal goal (weight loss / gain / muscle)
//   Level 6 — Preference (spice, dislikes)
// =============================================================================
...
import {
  ...
  resolveMedicationInteractions, // kept for backward-compat only
} from "./calorie-calculator";

// ── NEW: Import the rich medication rules system ─────────────────────────────
// This REPLACES resolveMedicationInteractions() as the primary medication handler.
// resolveMedicationInteractions() is kept so calorie-calculator.ts compiles clean.
import { resolveAllMedicationGuardrails } from "./src/lib/medicationRules";

// =============================================================================
// KNOWLEDGE BASE — Lookup tables baked into the engine.
// The AI never has to guess about food-drug or food-condition interactions.
// The backend tells it exactly what to avoid.
// =============================================================================

// ─────────────────────────────────────────────────────────────────────────────
// ALLERGY → FORBIDDEN INGREDIENTS MAP
// All Indian ingredient aliases included.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// RELIGIOUS / CULTURAL RULE → FORBIDDEN INGREDIENTS MAP
// ─────────────────────────────────────────────────────────────────────────────
...
const RELIGIOUS_FORBIDDEN_MAP: Record<string, string[]> = {
  ...
  jain_rules: [
    // Root vegetables (grown underground — forbidden in Jainism)
    ...
    // Also forbidden in many Jain sects
    "eggplant", "brinjal", "baingan",
  ],
};

// ─────────────────────────────────────────────────────────────────────────────
// CLINICAL CONDITION → DIETARY RULES
// ─────────────────────────────────────────────────────────────────────────────

interface ConditionDietaryRule {
  forbidden_ingredients: string[];
  limit_ingredients: string[];  // reduce, don't eliminate
  mandatory_nutrients: string[];
  max_sodium_mg_per_day?: number;
  special_instructions: string; // injected verbatim into Gemini prompt
}
...
// =============================================================================
// STEP 1: BUILD EFFECTIVE MEMBER PROFILES
// Merges FamilyMember (base) + MemberWeeklyContext (this-week override).
// Applies auto-assignment rules silently.
// =============================================================================
...
export function buildEffectiveProfiles(
  ...
  return members.map((member): EffectiveMemberProfile => {
    ...
    // Goal: weekly override > auto-assign > profile (respect goal_locked)
    ...
    // Weight: weekly update > profile (default to 0 if truly unknown)
    const effectiveWeightKg =
      weekly?.currentWeightKg ?? member.weightKg ?? 0;

    // Health conditions: weekly override replaces entirely if set
    ...
    // Fasting days: UNION of profile baseline + weekly additions
    ...
    // Calorie target — always returns a number (ICMR table fallback)
...
// =============================================================================
// STEP 2: CONFLICT DETECTION — all 6 priority levels
// =============================================================================
...
function detectConflicts(profiles: EffectiveMemberProfile[]): ConflictDetectorResult {
  ...
  // ── LEVEL 2: RELIGIOUS / DIETARY TYPE CONFLICTS ───────────────────────────
  ...
  // No beef / No pork cross-member check
  for (const p of profiles) {
  ...
  // ── LEVEL 3: MEDICATION CONFLICTS (detection only — resolution via med matrix) ──
  // Medication interaction strings are built in buildMedicationWarnings()
  // and appended directly to the ConstraintPacket.medicationWarnings.
  // Level 3 conflicts are registered here for the harmony score only.
  for (const p of profiles) {
    if (p.activeMedications.length > 0) {
      for (const med of p.activeMedications) {
        ...
        // No deduction for medication scheduling — it is correctly handled (+3 addition instead)
      }
    }
  }

  // ── LEVEL 4: CLINICAL CONDITION CONFLICTS ─────────────────────────────────
  ...
  // ── LEVEL 5: GOAL / CALORIE CONFLICTS ─────────────────────────────────────
  ...
  if (weightLossMembers.length > 0 && weightGainMembers.length > 0) {
    ...
    // Even without explicit goal conflict, flag large calorie spreads
    const calSpread =
      Math.max(...profiles.map((p) => p.dailyCalorieTarget)) -
      Math.min(...profiles.map((p) => p.dailyCalorieTarget));
    if (calSpread > 800) {
  ...
  // ── LEVEL 6: PREFERENCE / SPICE / DISLIKE CONFLICTS ──────────────────────
...
// Determines HOW each conflict is resolved. Produces the ResolutionRecord
// and the additions (harmony bonus points) for correctly handled constraints.
// =============================================================================
...
// =============================================================================
// STEP 4: HARMONY SCORE CALCULATION
// Formula is deterministic and explainable, per product spec.
// Start at 100. Deductions for constraints. Additions for correct scheduling.
// medicationBonus is now passed explicitly from buildMedicationWarnings()
// instead of being computed internally — the rule system owns that number.
// =============================================================================
...
function calculateHarmonyScore(
  ...
  medicationBonus: number = 0  // ← NEW parameter; defaults to 0 for backward-compat
): HarmonyScoreBreakdown {
  let score = 100;

  // Apply conflict deductions
  const totalDeducted = deductions.reduce((sum, d) => sum + d.points, 0);
  score += totalDeducted; // points are already negative

  // Pantry zero-waste bonus: +2 per perishable item queued (max 5 items × 2 = +10)
  ...
  // Medication scheduling bonus — now sourced from the rule system (+3 per drug class
  // correctly scheduled, as defined in MEDICATION_RULES[x].harmony_score_addition).
  // We convert the total into a single named addition for the breakdown display.
...
// =============================================================================
// STEP 5: NON-VEG DAY MAP
// Per-member map of days when non-veg meals are allowed.
// =============================================================================
...
function buildNonvegDayMap(
  ...
  for (const p of profiles) {
    if (p.dietaryType === "non_vegetarian") {
      // Non-veg every day unless specific days set this week
      result[p.id] =
        p.nonvegDaysThisWeek.length > 0 ? p.nonvegDaysThisWeek : ALL_DAYS;
    } else if (p.dietaryType === "occasional_non_veg") {
      // Use this week's days if set; else fall back to profile config; else none
    ...
    // Vegetarian / Jain / eggetarian members are not added to this map
  }
  return result;
}

// =============================================================================
// STEP 6: MEDICATION GUARDRAIL BUILDER
//
// SURGICAL CHANGE: This function now uses resolveAllMedicationGuardrails()
// from src/lib/medicationRules.ts instead of the old resolveMedicationInteractions().
//
// The new system produces structured MedicationGuardrailBundle objects that:
//   - Separate per-slot directives from weekly monitor directives
//   - Include scheduling notes for the waiting screen
//   - Track harmony score additions per drug
//   - Handle dynamic slot rules (e.g. Metformin at dinner instead of breakfast)
//   - Handle day-wide restrictions (e.g. Levothyroxine: soy-free all day)
//   - Handle cross-slot conflicts (e.g. Iron + Calcium → 2hr gap)
//   - Handle weekly consistency monitors (Warfarin, ACE inhibitors)
//
// Output is stored in both:
//   ConstraintPacket.medicationGuardrailBundles → structured (new)
//   ConstraintPacket.medicationWarnings         → flat strings (backward-compat)
// =============================================================================

interface MedicationResolutionResult {
  bundles: MedicationGuardrailBundle[];
  flatWarnings: string[];             // backward-compat: all directives flattened
  weeklyMonitorDirectives: string[];  // top-of-prompt weekly consistency rules
  schedulingNotes: string[];          // for generation log / waiting screen
  totalHarmonyAddition: number;       // sum of all bundle.harmony_score_addition
}
...
function buildMedicationWarnings(
  ...
  for (const profile of profiles) {
    ...
    // resolveAllMedicationGuardrails does the full pipeline:
    //   medication name → keyword match → regex match → rule lookup
    //   → parseMedicationTiming → buildGuardrailStrings
    ...
    for (const bundle of memberBundles) {
      ...
      // Flatten directives for backward-compat medicationWarnings array
      flatWarnings.push(...bundle.directives);

      // Collect weekly monitors separately — they go at the TOP of Call 2 prompt
      weeklyMonitorDirectives.push(...bundle.weekly_monitor_directives);

      // Collect scheduling notes for the waiting screen log
      schedulingNotes.push(...bundle.scheduling_notes);

      // Accumulate harmony score additions
...
// =============================================================================
// MAIN EXPORT: runConflictEngine
// Full orchestration: data → profiles → conflicts → resolutions → harmony → packet
// This is the single function called by the meal generation service.
// =============================================================================
...
export function runConflictEngine(params: {
  ...
  // 1. Build effective profiles (merge base + weekly overrides)
  ...
  // 4. Zero-waste pantry items
  const pantryZeroWasteItems = (weeklyContext.pantrySnapshot as PantryItem[]).filter(
    (i) => i.is_perishable
  );

  // 5. Fasting pre-load instructions
  const fastingPreloadInstructions = buildFastingPreloadInstructions(effectiveProfiles);

  // 6. ── MEDICATION GUARDRAILS (SURGICAL CHANGE) ───────────────────────────
  // buildMedicationWarnings() now returns structured MedicationResolutionResult,
  // not just a flat string[]. All new fields are populated and stored separately
  // in the ConstraintPacket so the prompt chain can inject them precisely.
  const medResult = buildMedicationWarnings(effectiveProfiles);

  // 7. Harmony score — now includes the medication harmony addition from rules
  //    medResult.totalHarmonyAddition replaces the old flat "+3 per medicated member"
  const harmonyScore = calculateHarmonyScore(
    ...
    medResult.totalHarmonyAddition  // ← NEW: pass medication bonus explicitly
  );

  // 8. Effective daily budget (weekly override / 7, else calculated)
  ...
  return {
    ...
    // ── Medication fields (all populated from new system) ──────────────────
    medicationWarnings: medResult.flatWarnings,                      // backward-compat
    medicationGuardrailBundles: medResult.bundles,                   // structured (new)
    medicationWeeklyMonitorDirectives: medResult.weeklyMonitorDirectives, // week-level rules
    medicationSchedulingNotes: medResult.schedulingNotes,            // waiting screen
    medicationHarmonyAddition: medResult.totalHarmonyAddition,       // score contribution
    // ──────────────────────────────────────────────────────────────────────
...
// =============================================================================
// UI TRANSPARENCY FORMATTER
// "We detected 3 conflicts. Here is how NutriNext resolved them."
// Produces the conflict card data for the frontend.
// =============================================================================
...
export function formatConflictSummaryForUI(
  ...
  const { harmonyScore, conflicts, resolutions } = packet;
  const score = harmonyScore.final_score;

  let label = "Excellent";
  if (score < 60) label = "Challenging";
  ...
  return {
    ...
    conflict_cards: conflicts
      .filter((c) => c.priority_level <= 5) // Show levels 1–5 on UI card
      .map((conflict, i) => {
