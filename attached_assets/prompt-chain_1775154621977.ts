// =============================================================================
// NutriNext ParivarSehat — 3-Step Gemini Prompt Chain
//
// API Call 1 → Monthly staples (40% of budget)
// API Call 2 → 7-day / 21-meal plan with "One Base, Many Plates" (50%)
// API Call 3 → Buffer dry fruits + seasonal fruit budget (10%)
//
// Uses Gemini REST API directly (no SDK) for Replit compatibility.
// All prompts are injected with constraint data from the conflict engine —
// Gemini never sees raw user input, only pre-validated, resolved constraints.
//
// Set GEMINI_API_KEY in your .env / Replit secrets.
// =============================================================================
...
//
// Uses the official @google/genai package (not the legacy @google/generative-ai).
// Install: npm install @google/genai
//
// Model: gemini-2.5-flash
//   - Fastest Gemini 2.5 variant. Perfect for JSON generation tasks.
//   - responseMimeType: "application/json" forces structured output.
//   - temperature: 0.3 → low randomness = deterministic, fewer hallucinations.
//
// REPLIT NOTE: GEMINI_API_KEY must be set in Replit Secrets, not .env.
// ─────────────────────────────────────────────────────────────────────────────
...
// Singleton — created once per process, reused across all 3 API calls
...
async function callGemini(prompt: string, maxOutputTokens = 8192): Promise<string> {
  ...
  const response = await ai.models.generateContent({
    ...
    config: {
      maxOutputTokens,
      temperature: 0.3,      // Low = deterministic, near-zero hallucinations
      topP: 0.8,
      responseMimeType: "application/json", // Forces JSON-only output — no prose, no code fences
    },
  });

  // Extract text from response — SDK provides .text getter on the response object
  // Fallback to manual extraction if SDK version differs
  ...
  // Belt-and-suspenders: strip any stray markdown fences if Gemini ignores responseMimeType
  return rawText
    .replace(/^```json\s*/im, "")
    .replace(/^```\s*/im, "")
    .replace(/\s*```\s*$/im, "")
...
// Shared building blocks reused across all 3 API calls.
// ─────────────────────────────────────────────────────────────────────────────
...
function buildConstraintInstructionSection(packet: ConstraintPacket): string {
  const {
    ...
    // ── MEDICATION (new structured fields) ─────────────────────────────────
    medicationGuardrailBundles,
    medicationWeeklyMonitorDirectives,
    // ── backward-compat flat warnings (still populated) ────────────────────
    medicationWarnings,
    weeklyContext,
  } = packet;
  ...
  // ── NEW: Week-level medication monitors injected FIRST ──────────────────
  // These govern the ENTIRE 7-day plan. Gemini must read these before
  // generating any individual day, otherwise it may violate a weekly rule
  // (e.g. Warfarin consistency, grapefruit zero-tolerance) mid-week.
  ...
  // ── NEW: Per-slot medication directives, grouped by member ─────────────
  // Uses the structured bundles if available; falls back to flat warnings.
  ...
  if (medicationGuardrailBundles && medicationGuardrailBundles.length > 0) {
    ...
    // Backward-compat path (if bundles somehow empty)
    medicationSection =
  ...
  // ── ASSEMBLY: Weekly monitors go at the very top, before per-slot rules ──
  return `
...
// ─────────────────────────────────────────────────────────────────────────────
// API CALL 1 — Monthly Staples List (40% budget)
// ─────────────────────────────────────────────────────────────────────────────
...
export async function generateStaplesList(packet: ConstraintPacket): Promise<{
  ...
  // Global never-ingredients (allergies + religious combined across all members)
  ...
  // Health-driven staple additions
  const allConditions = [
...
// ─────────────────────────────────────────────────────────────────────────────
// API CALL 2 — 7-Day / 21-Meal Plan with Plate-Level Instructions (50% budget)
// ─────────────────────────────────────────────────────────────────────────────
...
export async function generateWeeklyMealPlan(
  ...
  // Member ID reference for Gemini
...
// ─────────────────────────────────────────────────────────────────────────────
// API CALL 3 — Buffer: Dry Fruits + Seasonal Fruit Allocation (10% budget)
// ─────────────────────────────────────────────────────────────────────────────
...
export async function generateBufferList(packet: ConstraintPacket): Promise<{
  ...
  // Dry fruit exclusions from allergies
...
// ─────────────────────────────────────────────────────────────────────────────
// ORCHESTRATOR: runPromptChain
// Executes all 3 API calls sequentially and aggregates results.
// ─────────────────────────────────────────────────────────────────────────────
